...
from serializers import business_detailsSerializer,categorySerializer,ProfileSerializer
...
def Home(request):
        if request.method == "POST":
            movie_form = business_detailsForm(request.POST,request.FILES)
        ...
        movies = business_details.objects.all()
        print("moviess", movies)
        code = Transactions.objects.all()
        print("", code)
        context={"movie_form":movie_form,"movie":movies, "code": code}
...
def tablelist(request):
     ...
     context={"movie":movies, "codes": codes}
     print(context)
     for i in movies:
       print(i.image1)
